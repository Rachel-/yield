...
namespace :yield do
  namespace :attachments do
    ...
    desc 'Moves attachments stored at the root of the file directory (ie. created before Yield 2.3) to their subdirectories'
  ...
  task :plugins do
    Rake::Task["yield:plugins:migrate"].invoke
    Rake::Task["yield:plugins:assets"].invoke
  end

desc <<-DESC
FOR EXPERIMENTAL USE ONLY, Moves Yield data from production database to the development database.
...
DESC
  ...
  namespace :plugins do
    ...
    task :migrate => :environment do
      ...
      begin
        Yield::Plugin.migrate(name, version)
      rescue Yield::PluginNotFound
    ...
    task :assets => :environment do
      ...
      begin
        Yield::Plugin.mirror_assets(name)
      rescue Yield::PluginNotFound
        abort "Plugin #{name} was not found."
      end
    end
    ...
    task :test do
      Rake::Task["yield:plugins:test:units"].invoke
      Rake::Task["yield:plugins:test:functionals"].invoke
      Rake::Task["yield:plugins:test:integration"].invoke
    end
