...
module Yield
  module Hook
    ...
    class << self
      ...
      # Automatically called when a class inherits from Yield::Hook::Listener.
    ...
    class ViewListener < Listener
      ...
      #
      #   class MyHook < Yield::Hook::ViewListener
    ...
    module Helper
      def call_hook(hook, context={})
        if is_a?(ActionController::Base)
          ...
          Yield::Hook.call_hook(hook, default_context.merge(context))
        else
          ...
          Yield::Hook.call_hook(hook, default_context.merge(context)).join(' ').html_safe
        end
      end
    end
  end
...
ApplicationHelper.send(:include, Yield::Hook::Helper)
ActionController::Base.send(:include, Yield::Hook::Helper)
