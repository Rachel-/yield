    ...
    module Markdown
        ...
        def block_code(code, language)
          if language.present?
            "<pre><code class=\"#{CGI.escapeHTML language} syntaxhl\">" +
              Yield::SyntaxHighlighting.highlight_by_language(code, language) +
              "</code></pre>"
          else
      ...
      class Formatter
        ...
        def to_html(*args)
          ...
          # restore Yield links with double-quotes, eg. version:"1.0"
          html.gsub!(/(\w):&quot;(.+?)&quot;/) do
            "#{$1}:\"#{$2}\""
          end
          html
        ...
        def update_section(index, update, hash=nil)
          ...
          if hash.present? && hash != Digest::MD5.hexdigest(t[1])
            raise Yield::WikiFormatting::StaleSectionError
          end
        ...
        def formatter
          @@formatter ||= Redcarpet::Markdown.new(
            Yield::WikiFormatting::Markdown::HTML.new(
              :filter_html => true,
              :hard_wrap => true
            ),
            :autolink => true,
