    ...
    module Macros
      module Definitions
        ...
        def macro_exists?(name)
          Yield::WikiFormatting::Macros.available_macros.key?(name.to_sym)
        end

        def exec_macro(name, obj, args, text)
          macro_options = Yield::WikiFormatting::Macros.available_macros[name.to_sym]
      ...
      macro :collapse do |obj, args, text|
        html_id = "collapse-#{Yield::Utils.random_hex(4)}"
