    ...
    module PDF
      ...
      class ITCPDF < RBPDF
        ...
        def initialize(lang, orientation='P')
          ...
          set_creator(Yield::Info.app_name)
        ...
        def formatted_text(text)
          html = Yield::WikiFormatting.to_html(Setting.text_formatting, text)
          # Strip {{toc}} tags
          html.gsub!(/<p>\{\{([<>]?)toc\}\}<\/p>/i, '')
          html
        end
      ...
      class RDMPdfEncoding
        def self.rdm_from_utf8(txt, encoding)
          txt ||= ''
          txt = Yield::CodesetUtil.from_utf8(txt, encoding)
        ...
        def self.attach(attachments, filename, encoding)
          filename_utf8 = Yield::CodesetUtil.to_utf8(filename, encoding)
          atta = nil
