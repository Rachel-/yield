...
module Yield #:nodoc:
  ...
  # Base class for Yield plugins.
  # Plugins are registered using the <tt>register</tt> class method that acts as the public constructor.
  #
  #   Yield::Plugin.register :example do
  #     name 'Example plugin'
  #     author 'John Smith'
  #     description 'This is an example plugin for Yield'
  #     version '0.0.1'
  ...
  class Plugin
    ...
    # Sets a requirement on Yield version
    # Raises a PluginRequirementError exception if the requirement is not met
    #
    # Examples
    #   # Requires Yield 0.7.3 or higher
    #   requires_yield :version_or_higher => '0.7.3'
    #   requires_yield '0.7.3'
    #
    #   # Requires Yield 0.7.x or higher
    #   requires_yield '0.7'
    #
    #   # Requires a specific Yield version
    #   requires_yield :version => '0.7.3'              # 0.7.3 only
    #   requires_yield :version => '0.7'                # 0.7.x
    #   requires_yield :version => ['0.7.3', '0.8.0']   # 0.7.3 or 0.8.0
    #
    #   # Requires a Yield version within a range
    #   requires_yield :version => '0.7.3'..'0.9.1'     # >= 0.7.3 and <= 0.9.1
    #   requires_yield :version => '0.7'..'0.9'         # >= 0.7.x and <= 0.9.x
    def requires_yield(arg)
      arg = { :version_or_higher => arg } unless arg.is_a?(Hash)
      arg.assert_valid_keys(:version, :version_or_higher)

      current = Yield::VERSION.to_a
      arg.each do |k, req|
        case k
        when :version_or_higher
          ...
          unless compare_versions(req, current) <= 0
            raise PluginRequirementError.new("#{id} plugin requires Yield #{req} or higher but current is #{current.join('.')}")
          end
        when :version
          req = [req] if req.is_a?(String)
          if req.is_a?(Array)
            unless req.detect {|ver| compare_versions(ver, current) == 0}
              raise PluginRequirementError.new("#{id} plugin requires one the following Yield versions: #{req.join(', ')} but current is #{current.join('.')}")
            end
          elsif req.is_a?(Range)
            unless compare_versions(req.first, current) <= 0 && compare_versions(req.last, current) >= 0
              raise PluginRequirementError.new("#{id} plugin requires a Yield version between #{req.first} and #{req.last} but current is #{current.join('.')}")
            end
          else
    ...
    # Sets a requirement on a Yield plugin version
    # Raises a PluginRequirementError exception if the requirement is not met
    #
    # Examples
    #   # Requires a plugin named :foo version 0.7.3 or higher
    #   requires_yield_plugin :foo, :version_or_higher => '0.7.3'
    #   requires_yield_plugin :foo, '0.7.3'
    #
    #   # Requires a specific version of a Yield plugin
    #   requires_yield_plugin :foo, :version => '0.7.3'              # 0.7.3 only
    #   requires_yield_plugin :foo, :version => ['0.7.3', '0.8.0']   # 0.7.3 or 0.8.0
    def requires_yield_plugin(plugin_name, arg)
      arg = { :version_or_higher => arg } unless arg.is_a?(Hash)
      arg.assert_valid_keys(:version, :version_or_higher)
    ...
    def menu(menu, item, url, options={})
      Yield::MenuManager.map(menu).push(item, url, options)
    end
    alias :add_menu_item :menu

    # Removes +item+ from the given +menu+.
    def delete_menu_item(menu, item)
      Yield::MenuManager.map(menu).delete(item)
    end
    ...
    def permission(name, actions, options = {})
      if @project_module
        Yield::AccessControl.map {|map| map.project_module(@project_module) {|map|map.permission(name, actions, options)}}
      else
        Yield::AccessControl.map {|map| map.permission(name, actions, options)}
      end
    end
    ...
    def wiki_format_provider(name, formatter, helper)
      Yield::WikiFormatting.register(name, formatter, helper)
    end
    ...
    def migrate(version = nil)
      ...
      Yield::Plugin::Migrator.migrate_plugin(self, version)
    end
