...
module Yield
  module Configuration
    ...
    class << self
      # Loads the Yield configuration file
      # Valid options:
      ...
      def load_from_yaml(filename, env)
        ...
        begin
          ...
          $stderr.puts "Your Yield configuration file located at #{filename} is not a valid YAML file and could not be loaded."
          exit 1
        rescue SyntaxError => e
          $stderr.puts "A syntax error occurred when parsing your Yield configuration file located at #{filename} with ERB:\n#{e.message}"
          exit 1
        end
        conf = {}
        if yaml.is_a?(Hash)
          ...
          $stderr.puts "Your Yield configuration file located at #{filename} is not a valid Yield configuration file."
          exit 1
        end
        conf
      end
