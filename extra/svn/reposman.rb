...
optparse = OptionParser.new do |opts|
  ...
  opts.separator("")
  opts.separator("Manages your repositories with Yield.")
  opts.separator("")
  opts.separator("Required arguments:")
  opts.on("-s", "--svn-dir DIR",      "use DIR as base directory for svn repositories") {|v| $repos_base = v}
  opts.on("-r", "--yield-host HOST","assume Yield is hosted on HOST. Examples:",
                                       " -r yield.example.net",
                                       " -r http://yield.example.net",
                                       " -r https://yield.example.net") {|v| $yield_host = v}
  opts.on("-k", "--key KEY",           "use KEY as the Yield API key",
  ...
  opts.on("-o", "--owner OWNER",       "owner of the repository. using the rails login",
                                       ...
                                       "Yield even for private projects. If you want to",
                                       "share repositories through Yield.pm, you need",
  ...
  opts.on(      "--key-file FILE",     "path to a file that contains the Yield API key",
  ...
  opts.on("-f", "--force",             "force repository creation even if the project", "repository is already declared in Yield") {$force = true}
  ...
  opts.separator("  reposman.rb --svn-dir=/var/svn --yield-host=yield.host")
  opts.separator("  reposman.rb -s /var/git -r yield.host -u http://git.host --scm git")
  opts.separator("")
  opts.separator("You can find more information on the yield's wiki:\nhttp://www.yield.org/projects/yield/wiki/HowTos")
...
if ($yield_host.empty? or $repos_base.empty?)
...
class Project < ActiveResource::Base
  self.headers["User-agent"] = "Yield repository manager/#{Version}"
  self.format = :xml
end

log("querying Yield for projects...", :level => 1);

$yield_host.gsub!(/^/, "http://") unless $yield_host.match("^https?://")
$yield_host.gsub!(/\/$/, '')

Project.site = "#{$yield_host}/sys";

begin
  # Get all active projects that have the Repository module enabled
  projects = Project.find(:all, :params => {:key => $api_key})
rescue ActiveResource::ForbiddenAccess
  log("Request was denied by your Yield server. Make sure that 'WS for repository management' is enabled in application settings and that you provided the correct API key.")
rescue => e
...
projects.each do |project|
  ...
  if File.directory?(repos_path)
    ...
    # if repository is already declared in yield, we don't create
    # unless user use -f with reposman
    if $force == false and project.respond_to?(:repository)
      log("\trepository for project #{project.identifier} already exists in Yield", :level => 1)
      next
    end
    ...
    if $test
      ...
      log("\trepository #{repos_path} registered in Yield with url #{$svn_url}#{project.identifier}") if $svn_url;
      next
    end
    ...
    if $svn_url
      begin
        ...
        log("\trepository #{repos_path} registered in Yield with url #{$svn_url}#{project.identifier}");
      rescue => e
        log("\trepository #{repos_path} not registered in Yield: #{e.message}");
      end
    end
