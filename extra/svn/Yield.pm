package Apache::Authn::Yield;

=head1 Apache::Authn::Yield

Yield - a mod_perl module to authenticate webdav subversion users
against yield database

=head1 SYNOPSIS

This module allow anonymous users to browse public project and
registred users to browse and commit their project. Authentication is
done against the yield database or the LDAP configured in yield.
...
If your Yield users use LDAP authentication, you will also need
...
=head1 CONFIGURATION
   ...
   ## eg:  /usr/lib/perl5/Apache/Authn/Yield.pm
   PerlLoadModule Apache::Authn::Yield
   <Location /svn>
     DAV svn
     ...
     PerlAccessHandler Apache::Authn::Yield::access_handler
     PerlAuthenHandler Apache::Authn::Yield::authen_handler

     ## for mysql
     YieldDSN "DBI:mysql:database=databasename;host=my.db.server"
     ## for postgres
     # YieldDSN "DBI:Pg:dbname=databasename;host=my.db.server"

     YieldDbUser "yield"
     YieldDbPass "password"
     ## Optional where clause (fulltext search would be slow and
     ## database dependant).
     # YieldDbWhereClause "and members.role_id IN (1,2)"
     ## Optional credentials cache size
     # YieldCacheCredsMax 50
  </Location>

To be able to browse repository inside yield, you must add something
like that :
   ...
   <Location /svn-private>
     ...
     <Limit GET PROPFIND OPTIONS REPORT>
       Allow from yield.server.ip
     </Limit>
   </Location>

and you will have to use this reposman.rb command line to create repository :

  reposman.rb --yield my.yield.server --svn-dir /var/svn --owner www-data -u http://svn.server/svn-private/
...
above settings. Yield.pm normally does access control depending on the HTTP
...
a MySQL database and this new option:
   ...
   <Location /git>
       ...
       PerlAuthenHandler Apache::Authn::Yield::authen_handler
       # for mysql
       YieldDSN "DBI:mysql:database=yield;host=127.0.0.1"
       YieldDbUser "yield"
       YieldDbPass "xxx"
       YieldGitSmartHttp yes
    </Location>
...
recommended. You should create them with the rights of the user running Yield,
like this:

  cd /var/www/git
  sudo -u user-running-yield mkdir myproject
  cd myproject
  sudo -u user-running-yield git init --bare
...
my @directives = (
  ...
  {
    name => 'YieldDbWhereClause',
    req_override => OR_AUTHCFG,
    args_how => TAKE1,
  },
  {
    name => 'YieldCacheCredsMax',
    req_override => OR_AUTHCFG,
    args_how => TAKE1,
    errmsg => 'YieldCacheCredsMax must be decimal number',
  },
  {
    name => 'YieldGitSmartHttp',
    req_override => OR_AUTHCFG,
    args_how => TAKE1,
  },
);
...
sub YieldDSN {
  ...
  $self->{YieldQuery} = trim($query);
}

sub YieldDbUser { set_val('YieldDbUser', @_); }
sub YieldDbPass { set_val('YieldDbPass', @_); }
sub YieldDbWhereClause {
  my ($self, $parms, $arg) = @_;
  $self->{YieldQuery} = trim($self->{YieldQuery}.($arg ? $arg : "")." ");
}

sub YieldCacheCredsMax {
  my ($self, $parms, $arg) = @_;
  if ($arg) {
    $self->{YieldCachePool} = APR::Pool->new;
    $self->{YieldCacheCreds} = APR::Table::make($self->{YieldCachePool}, $arg);
    $self->{YieldCacheCredsCount} = 0;
    $self->{YieldCacheCredsMax} = $arg;
  }
}

sub YieldGitSmartHttp {
  my ($self, $parms, $arg) = @_;
  $arg = lc $arg;

  if ($arg eq "yes" || $arg eq "true") {
    $self->{YieldGitSmartHttp} = 1;
  } else {
    $self->{YieldGitSmartHttp} = 0;
  }
}
...
sub request_is_read_only {
  ...
  if (defined $cfg->{YieldGitSmartHttp} and $cfg->{YieldGitSmartHttp}) {
...
sub authen_handler {
  ...
  my ($res, $yield_pass) =  $r->get_basic_auth_pw();
  return $res unless $res == OK;

  if (is_member($r->user, $yield_pass, $r)) {
      return OK;
  } else {
      $r->note_auth_failure();
      return DECLINED;
...
sub is_member {
  ...
  my $pass_digest = Digest::SHA::sha1_hex($yield_pass);

  my $access_mode = request_is_read_only($r) ? "R" : "W";

  my $cfg = Apache2::Module::get_config(__PACKAGE__, $r->server, $r->per_dir_config);
  my $usrprojpass;
  if ($cfg->{YieldCacheCredsMax}) {
    $usrprojpass = $cfg->{YieldCacheCreds}->get($yield_user.":".$project_id.":".$access_mode);
    return 1 if (defined $usrprojpass and ($usrprojpass eq $pass_digest));
  }
  my $query = $cfg->{YieldQuery};
  my $sth = $dbh->prepare($query);
  $sth->execute($yield_user, $project_id);
  ...
  while (my ($hashed_password, $salt, $auth_source_id, $permissions, $project_status) = $sth->fetchrow_array) {
      ...
      unless ($auth_source_id) {
          ...
          while (my @rowldap = $sthldap->fetchrow_array) {
            ...
            if ($bind_as =~ m/\$login/) {
              # replace $login with $yield_user and use $yield_pass
              $bind_as =~ s/\$login/$yield_user/g;
              $bind_pw = $yield_pass
            }
            ...
            my $method = $r->method;
            $ret = 1 if ($ldap->authenticate($yield_user, $yield_pass) && (($access_mode eq "R" && $permissions =~ /:browse_repository/) || $permissions =~ /:commit_access/));
  ...
  if ($cfg->{YieldCacheCredsMax} and $ret) {
    if (defined $usrprojpass) {
      $cfg->{YieldCacheCreds}->set($yield_user.":".$project_id.":".$access_mode, $pass_digest);
    } else {
      if ($cfg->{YieldCacheCredsCount} < $cfg->{YieldCacheCredsMax}) {
        $cfg->{YieldCacheCreds}->set($yield_user.":".$project_id.":".$access_mode, $pass_digest);
        $cfg->{YieldCacheCredsCount}++;
      } else {
        $cfg->{YieldCacheCreds}->clear();
        $cfg->{YieldCacheCredsCount} = 0;
      }
    }
  }
...
sub get_project_identifier {
    ...
    my $cfg = Apache2::Module::get_config(__PACKAGE__, $r->server, $r->per_dir_config);
    my $location = $r->location;
    $location =~ s/\.git$// if (defined $cfg->{YieldGitSmartHttp} and $cfg->{YieldGitSmartHttp});
...
sub connect_database {
    ...
    my $cfg = Apache2::Module::get_config(__PACKAGE__, $r->server, $r->per_dir_config);
    return DBI->connect($cfg->{YieldDSN}, $cfg->{YieldDbUser}, $cfg->{YieldDbPass});
}
