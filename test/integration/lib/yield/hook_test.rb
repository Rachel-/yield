...
class HookTest < ActionController::IntegrationTest
  ...
  class ProjectBasedTemplate < Yield::Hook::ViewListener
  ...
  class SidebarContent < Yield::Hook::ViewListener
    def view_layouts_base_sidebar(context)
      content_tag('p', 'Sidebar hook')
    end
  end

  class ContentForInsideHook < Yield::Hook::ViewListener
...
VIEW
  ...
  class ContextTestHook < Yield::Hook::ViewListener
  ...
  def test_html_head_hook_response
    Yield::Hook.add_listener(ProjectBasedTemplate)
  ...
  def test_sidebar_with_hook_content_should_not_be_hidden
    Yield::Hook.add_listener(SidebarContent)
  ...
  def test_hook_with_content_for_should_append_content
    Yield::Hook.add_listener(ContentForInsideHook)
  ...
  def test_controller_hook_context_should_include_request
    Yield::Hook.add_listener(ContextTestHook)
