...
module YieldMenuTestHelper
  # Helpers
  def get_menu_item(menu_name, item_name)
    Yield::MenuManager.items(menu_name).find {|item| item.name == item_name.to_sym}
  end
end

class Yield::MenuManager::MenuItemTest < ActiveSupport::TestCase
  include YieldMenuTestHelper

  Yield::MenuManager.map :test_menu do |menu|
  ...
  def test_new_menu_item_should_require_a_name
    assert_raises ArgumentError do
      Yield::MenuManager::MenuItem.new
    end
  end

  def test_new_menu_item_should_require_an_url
    assert_raises ArgumentError do
      Yield::MenuManager::MenuItem.new(:test_missing_url)
    end
  end

  def test_new_menu_item_should_require_the_options
    assert_raises ArgumentError do
      Yield::MenuManager::MenuItem.new(:test_missing_options, '/test')
    end
  end

  def test_new_menu_item_with_all_required_parameters
    assert Yield::MenuManager::MenuItem.new(:test_good_menu, '/test', {})
  end

  def test_new_menu_item_should_require_a_proc_to_use_for_the_if_condition
    assert_raises ArgumentError do
      Yield::MenuManager::MenuItem.new(:test_error, '/test',
                                         {
                                           :if => ['not_a_proc']
                                         })
    end

    assert Yield::MenuManager::MenuItem.new(:test_good_if, '/test',
                                              {
                                                :if => Proc.new{}
                                              })
  end
  ...
  def test_new_menu_item_should_allow_a_hash_for_extra_html_options
    ...
    assert Yield::MenuManager::MenuItem.new(:test_good_html, '/test',
                                              {
  ...
  def test_new_menu_item_should_require_a_proc_to_use_the_children_option
    ...
    assert Yield::MenuManager::MenuItem.new(:test_good_children, '/test',
                                              {
                                                :children => Proc.new{}
                                              })
  end
  ...
  def test_new_should_not_allow_setting_the_parent_item_to_the_current_item
    assert_raises ArgumentError do
      Yield::MenuManager::MenuItem.new(:test_error, '/test', { :parent => :test_error })
    end
  end
