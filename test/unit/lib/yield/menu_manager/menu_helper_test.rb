...
class Yield::MenuManager::MenuHelperTest < ActionView::TestCase

  include Yield::MenuManager::MenuHelper
  include ERB::Util
  ...
  def test_render_single_menu_node
    node = Yield::MenuManager::MenuItem.new(:testing, '/test', { })
  ...
  def test_render_menu_node
    single_node = Yield::MenuManager::MenuItem.new(:single_node, '/test', { })
  ...
  def test_render_menu_node_with_nested_items
    parent_node = Yield::MenuManager::MenuItem.new(:parent_node, '/test', { })
    parent_node << Yield::MenuManager::MenuItem.new(:child_one_node, '/test', { })
    parent_node << Yield::MenuManager::MenuItem.new(:child_two_node, '/test', { })
    parent_node <<
      Yield::MenuManager::MenuItem.new(:child_three_node, '/test', { }) <<
      Yield::MenuManager::MenuItem.new(:child_three_inner_node, '/test', { })
  ...
  def test_render_menu_node_with_children
    ...
    parent_node = Yield::MenuManager::MenuItem.new(:parent_node,
                                                     '/test',
                                                     {
                                                       :children => Proc.new {|p|
                                                         children = []
                                                         3.times do |time|
                                                           children << Yield::MenuManager::MenuItem.new("test_child_#{time}",
    ...
    parent_node = Yield::MenuManager::MenuItem.new(:parent_node,
                                                     '/test',
                                                     {
                                                       :children => Proc.new {|p|
                                                         children = []
                                                         3.times do |time|
                                                           children << Yield::MenuManager::MenuItem.new("test_child_#{time}", {:controller => 'issues', :action => 'index'}, {})
                                                         end
                                                         children
                                                       }
                                                     })

    parent_node << Yield::MenuManager::MenuItem.new(:child_node,
                                                     '/test',
                                                     {
                                                       :children => Proc.new {|p|
                                                         children = []
                                                         6.times do |time|
                                                            children << Yield::MenuManager::MenuItem.new("test_dynamic_child_#{time}", {:controller => 'issues', :action => 'index'}, {})
                                                         end
                                                         children
                                                       }
                                                     })
  ...
  def test_render_menu_node_with_children_without_an_array
    parent_node = Yield::MenuManager::MenuItem.new(:parent_node,
                                                     '/test',
                                                     {
                                                       :children => Proc.new {|p| Yield::MenuManager::MenuItem.new("test_child", "/testing", {})}
                                                     })

    assert_raises Yield::MenuManager::MenuError, ":children must be an array of MenuItems" do
      @output_buffer = render_menu_node(parent_node, Project.find(1))
    end
  end
  ...
  def test_menu_items_for_should_yield_all_items_if_passed_a_block
    ...
    Yield::MenuManager.map menu_name do |menu|
      menu.push(:a_menu, '/', { })
      menu.push(:a_menu_2, '/', { })
      menu.push(:a_menu_3, '/', { })
    end
