...
class Yield::AccessControlTest < ActiveSupport::TestCase

  def setup
    @access_module = Yield::AccessControl
  end

  def test_permissions
    perms = @access_module.permissions
    assert perms.is_a?(Array)
    assert perms.first.is_a?(Yield::AccessControl::Permission)
  end

  def test_module_permission
    perm = @access_module.permission(:view_issues)
    assert perm.is_a?(Yield::AccessControl::Permission)
