...
class Yield::WikiFormatting::MacrosTest < ActionView::TestCase
  ...
  def test_macro_registration
    Yield::WikiFormatting::Macros.register do
  ...
  def test_macro_name_with_upper_case
    Yield::WikiFormatting::Macros.macro(:UpperCase) {|obj, args| "Upper"}

    assert_equal "<p>Upper</p>", textilizable("{{UpperCase}}")
  end

  def test_multiple_macros_on_the_same_line
    Yield::WikiFormatting::Macros.macro :foo do |obj, args|
  ...
  def test_macro_exception_should_be_displayed
    Yield::WikiFormatting::Macros.macro :exception do |obj, args|
      raise "My message"
    end
  ...
  def test_macro_arguments_should_not_be_parsed_by_formatters
    text = '{{hello_world(http://www.yield.org, #1)}}'
    assert_include 'Arguments: http://www.yield.org, #1', textilizable(text)
  end
  ...
  def test_html_safe_macro_output_should_not_be_escaped
    Yield::WikiFormatting::Macros.macro :safe_macro do |obj, args|
      "<tag>".html_safe
    end
