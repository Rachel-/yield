...
class FilesystemAdapterTest < ActiveSupport::TestCase
  ...
  if File.directory?(REPOSITORY_PATH)
    def setup
      @adapter = Yield::Scm::Adapters::FilesystemAdapter.new(REPOSITORY_PATH)
    end
    ...
    def test_path_encoding_default_utf8
      adpt1 = Yield::Scm::Adapters::FilesystemAdapter.new(
                                  REPOSITORY_PATH
                                )
      assert_equal "UTF-8", adpt1.path_encoding
      adpt2 = Yield::Scm::Adapters::FilesystemAdapter.new(
                                  REPOSITORY_PATH,
                                  nil,
                                  nil,
                                  nil,
