...
begin
  ...
  class CvsAdapterTest < ActiveSupport::TestCase
    ...
    if File.directory?(REPOSITORY_PATH)
      def setup
        @adapter = Yield::Scm::Adapters::CvsAdapter.new(MODULE_NAME, REPOSITORY_PATH)
      end
      ...
      def test_path_encoding_default_utf8
        adpt1 = Yield::Scm::Adapters::CvsAdapter.new(
                                  MODULE_NAME,
                                  REPOSITORY_PATH
                                )
        assert_equal "UTF-8", adpt1.path_encoding
        adpt2 = Yield::Scm::Adapters::CvsAdapter.new(
                                  MODULE_NAME,
                                  REPOSITORY_PATH,
                                  nil,
                                  nil,
      ...
      def test_root_url_path
        ...
        to_test.each do |string, expected|
          assert_equal expected, Yield::Scm::Adapters::CvsAdapter.new('foo', string).send(:root_url_path), "#{string} failed"
        end
      end
