...
class Yield::UnifiedDiffTest < ActiveSupport::TestCase
  def test_subversion_diff
    diff = Yield::UnifiedDiff.new(read_diff_fixture('subversion.diff'))
    # number of files
  ...
  def test_truncate_diff
    diff = Yield::UnifiedDiff.new(read_diff_fixture('subversion.diff'), :max_lines => 20)
    assert_equal 2, diff.size
  end

  def test_inline_partials
    diff = Yield::UnifiedDiff.new(read_diff_fixture('partials.diff'))
  ...
  def test_side_by_side_partials
    diff = Yield::UnifiedDiff.new(read_diff_fixture('partials.diff'), :type => 'sbs')
    assert_equal 1, diff.size
    diff = diff.first
...
DIFF
    ...
    diff = Yield::UnifiedDiff.new(raw, :type => 'sbs')
    assert_equal 1, diff.size
    ...
    diff = Yield::UnifiedDiff.new(raw, :type => 'inline')
    assert_equal 1, diff.size
  ...
  def test_line_starting_with_dashes
    diff = Yield::UnifiedDiff.new(<<-DIFF
...
DIFF
  ...
  def test_utf8_ja
    ...
    with_settings :repositories_encodings => '' do
      diff = Yield::UnifiedDiff.new(read_diff_fixture('issue-12641-ja.diff'), :type => 'inline')
      assert_equal 1, diff.size
  ...
  def test_utf8_ru
    ...
    with_settings :repositories_encodings => '' do
      diff = Yield::UnifiedDiff.new(read_diff_fixture('issue-12641-ru.diff'), :type => 'inline')
      assert_equal 1, diff.size
