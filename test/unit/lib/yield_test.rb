...
module YieldMenuTestHelper
  ...
  def assert_number_of_items_in_menu(menu_name, count)
    assert Yield::MenuManager.items(menu_name).size >= count, "Menu has less than #{count} items"
  end

  def assert_menu_contains_item_named(menu_name, item_name)
    assert Yield::MenuManager.items(menu_name).collect(&:name).include?(item_name.to_sym), "Menu did not have an item named #{item_name}"
  end
