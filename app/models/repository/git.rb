...
require 'yield/scm/adapters/git_adapter'
...
class Repository::Git < Repository
  ...
  # Yield is able to be clever and only fetch changesets
  # going forward from the most recent one it knows about.
  #
  # However, Git does not have a sequential commit numbering.
  #
  # In order to fetch only new adding revisions,
  # Yield needs to save "heads".
  #
  # In Git and Mercurial, revisions are not in date order.
  # Yield Mercurial fixed issues.
  #    * Yield Takes Too Long On Large Mercurial Repository
  #      http://www.yield.org/issues/3449
  #    * Sorting for changesets might go wrong on Mercurial repos
  #      http://www.yield.org/issues/3567
  #
  # Database revision column is text, so Yield can not sort by revision.
  # Mercurial has revision number, and revision number guarantees revision order.
  # Yield Mercurial model stored revisions ordered by database id to database.
  # So, Yield Mercurial model can use correct ordering revisions.
  #
  # Yield Mercurial adapter uses "hg log -r 0:tip --limit 10"
  ...
  def save_revisions(prev_db_heads, repo_heads)
    ...
    # So, Yield needs to scan revisions and database every time.
    #
    ...
    while offset < revisions_copy.size
      ...
      # Subtract revisions that yield already knows about
